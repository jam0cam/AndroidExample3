 ...
 *
 * We will not need this helper after Kotlin fix this issue: https://youtrack.jetbrains.com/issue/KT-14984
 *
 * copied from https://github.com/ReactiveX/RxKotlin/blob/2.x/src/main/kotlin/io/reactivex/rxkotlin/Observables.kt
 */
object Observables {
...
inline fun <reified T : Any?> T?.toMaybe(): Maybe<T> {
    return this?.let { Maybe.just(it) } ?: Maybe.empty()
}

inline fun <reified T> Observable<T>.withSchedulers(): Observable<T> {
    return this
        .subscribeOn(Schedulers.io())
        .observeOn(AndroidSchedulers.mainThread())
}

inline fun <reified T> Maybe<T>.withSchedulers(): Maybe<T> {
    return this
        .subscribeOn(Schedulers.io())
        .observeOn(AndroidSchedulers.mainThread())
}

inline fun <reified T> Single<T>.withSchedulers(): Single<T> {
    return this
        .subscribeOn(Schedulers.io())
        .observeOn(AndroidSchedulers.mainThread())
}
...
fun <T> T.asSingle(): Single<T> = Single.fromCallable { this }